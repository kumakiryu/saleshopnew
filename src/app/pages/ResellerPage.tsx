        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <div className="flex items-center gap-3 mb-6">
              <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl" style={{ background: 'rgba(0,230,118,0.12)', border: '1px solid rgba(0,230,118,0.25)', color: GREEN }}>◆</div>
              <div>
            ...
            <p className="text-sm leading-relaxed mb-8" style={{ color: '#7b88c0' }}>Become an official reseller and access the lowest prices on the platform. Earn double tokens on every purchase.</p>
            ...
            <div className="mt-6 p-4 rounded-xl" style={{ background: 'rgba(0,230,118,0.04)', border: '1px solid rgba(0,230,118,0.15)' }}>
              ...
              <p className="text-xs" style={{ color: '#7b88c0' }}>Open a ticket on our Discord server and request a reseller account. Our team will review your application — usually within 24 hours.</p>
            </div>
          </motion.div>

          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
            {user ? (
              <div className="p-6 rounded-2xl" style={{ background: 'linear-gradient(135deg, rgba(0,230,118,0.05) 0%, rgba(255,255,255,0.02) 100%)', border: `1px solid ${isReseller ? 'rgba(0,230,118,0.3)' : 'rgba(255,255,255,0.07)'}` }}>
                {isReseller ? (
                  <>
                    ...
                    <div className="mb-4 p-3 rounded-xl flex items-center justify-between" style={{ background: 'rgba(0,230,118,0.06)', border: '1px solid rgba(0,230,118,0.2)' }}>
                      ...
                      <button onClick={() => navigate('/reseller/topup')} className="text-xs px-3 py-1.5 rounded-lg font-bold" style={{ background: 'rgba(0,230,118,0.12)', border: '1px solid rgba(0,230,118,0.3)', color: GREEN, cursor: 'pointer' }}>Top Up</button>
                    </div>
                    <div className="flex flex-col gap-2">
                      <button onClick={() => navigate('/reseller/dashboard')} className="w-full py-3 rounded-xl text-sm font-bold tracking-wider" style={{ background: 'linear-gradient(135deg, rgba(0,230,118,0.18) 0%, rgba(0,200,100,0.18) 100%)', border: '1px solid rgba(0,230,118,0.4)', color: GREEN, fontFamily: "'Rajdhani', 'Inter', sans-serif", cursor: 'pointer' }}>MY DASHBOARD →</button>
                      <button onClick={() => navigate('/stock')} className="w-full py-2.5 rounded-xl text-sm font-semibold" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', color: '#7b88c0', cursor: 'pointer' }}>Shop with Reseller Prices</button>
                    </div>
                  </>
                ) : (
                  <div className="text-center">
                    <p className="text-sm mb-2" style={{ color: '#c8d0f0' }}>Logged in as <strong>{user.email}</strong></p>
                    <p className="text-xs mb-4" style={{ color: '#7b88c0' }}>{isVip ? 'You have VIP membership.' : 'Open a ticket on Discord to apply for Reseller.'}</p>
              ...
              <>
                <div className="p-6 rounded-2xl" style={{ background: 'linear-gradient(135deg, rgba(255,255,255,0.04) 0%, rgba(255,255,255,0.02) 100%)', border: '1px solid rgba(255,255,255,0.07)' }}>
                  ...
                  <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                    ...
                    <button type="submit" disabled={loading} style={{ background: loading ? 'rgba(0,230,118,0.04)' : 'linear-gradient(135deg, rgba(0,230,118,0.18) 0%, rgba(0,200,100,0.18) 100%)', border: '1px solid rgba(0,230,118,0.4)', color: loading ? '#3a4570' : GREEN, padding: '12px', borderRadius: '12px', fontWeight: 700, letterSpacing: '0.08em', fontFamily: "'Rajdhani', 'Inter', sans-serif", fontSize: '14px', cursor: loading ? 'not-allowed' : 'pointer', width: '100%' }}>
                  ...
                  <div className="mt-5 pt-4" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                    <p className="text-[11px] text-center" style={{ color: '#3a4570' }}>
                      ...
                      <a href="https://discord.gg/2n5UZj56Nk" target="_blank" rel="noopener noreferrer" style={{ color: GREEN, textDecoration: 'none' }}>Open a ticket on Discord →</a>
                    </p>
                  </div>
                </div>
                <div className="mt-4 text-center">
                  <button onClick={() => navigate('/vip')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#7b88c0', fontSize: '12px', padding: 0 }}>
