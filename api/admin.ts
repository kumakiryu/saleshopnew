...
async function handleTokens(req: VercelRequest, res: VercelResponse) {
  ...
  const { action, email, token_type, amount } = req.body ?? {};
  if (!action || !email || !token_type) return res.status(400).json({ error: 'Missing fields' });
  ...
  if (!authUser?.id) return res.status(404).json({ error: `No user found with email: ${email}` });
  const target_user_id: string = authUser.id;
