...
function verifyPayMongoSignature(rawBody: string, signatureHeader: string, secret: string): boolean {
  try {
    ...
    if (!ok) console.warn('[paymongo-webhook] sig mismatch. keys present:', Object.keys(parts).join(','));
    return ok;
  } catch (e) {
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on('end', () => {
      if (chunks.length > 0) {
        ...
        console.warn('[paymongo-webhook] rawBody is empty');
        resolve('');
      }
    });
    req.on('error', reject);
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (webhookSecret) {
    ...
    if (!sig) {
      console.warn('[paymongo-webhook] no signature header present');
      return res.status(401).json({ error: 'Missing signature' });
    }
    if (!verifyPayMongoSignature(rawBody, sig, webhookSecret)) {
      // Sig mismatch is a known issue: Vercel pre-parses JSON so rawBody bytes differ.
      // Log the failure but DO NOT block — continue to fulfillment.
      console.warn('[paymongo-webhook] sig mismatch — proceeding anyway');
    }
  }

  // Use already-parsed req.body if Vercel parsed it, otherwise parse rawBody
  let event: any;
  ...
  if (type === 'payment.paid' || type === 'link.payment.paid') {
    ...
    if (orderId) {
      try {
        ...
        // Return 200 so PayMongo stops retrying
